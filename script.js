/* ==========================================================================
   ...
   Reads content dynamically from Server API (NO Local Storage)
   ========================================================================== */
...
/* --------------------------------------------------------------------------
   DATA MANAGEMENT (FETCH FROM SERVER - NO LOCAL STORAGE)
   -------------------------------------------------------------------------- */
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data && Object.keys(data).length > 0) {
                ...
                siteData.services = siteData.services && siteData.services.length ? siteData.services : DEFAULT_SITE_DATA.services;
                siteData.videos = siteData.videos && siteData.videos.length ? siteData.videos : DEFAULT_SITE_DATA.videos;
                siteData.gallery = siteData.gallery && siteData.gallery.length ? siteData.gallery : DEFAULT_SITE_DATA.gallery;
                siteData.reviews = siteData.reviews && siteData.reviews.length ? siteData.reviews : DEFAULT_SITE_DATA.reviews;
                siteData.whyUs = DEFAULT_SITE_DATA.whyUs;
                return;
            }
        }
    } catch (err) {
        console.warn('Unable to load from server API, using initial defaults:', err);
    }
